...
/// Among ready processes, execute the one with smallest burst time.
/// </summary>
public class SJFAlgorithm : ICPUScheduling
{
    ...
    public (List<Process>, List<(int, int)>, double, double, double) Execute()
    {
        ...
        while (completed < n)
        {
            ...
            // Find ready process with smallest burst time
            for (int i = 0; i < n; i++)
            {
                ...
                if (idx == -1)
                    idx = i;
                else if (_processes[i].BurstTime < _processes[idx].BurstTime)
                    idx = i;
                else if (_processes[i].BurstTime == _processes[idx].BurstTime && 
